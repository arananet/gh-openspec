...
gh openspec create my-api --no-clone   # create on GitHub only
```
...
```bash
gh openspec init
gh openspec init --force        # overwrite existing .openspec/
gh openspec init --skip-hooks   # don't run setup.sh
```
...
```bash
gh openspec check
gh openspec check --strict       # warnings become errors
...
spec:
  ...
  enforce_on_pr: true       # block PRs without spec changes
  enforce_on_commit: true   # block commits without spec changes
...
hooks:
  ...
  commit_msg:
    require_spec_reference: false  # set true to require "spec: <slug>" in messages
```
...
description: |
  Allow users to sign in with email/password. Focus on the outcome:
  authenticated session with a JWT, not the implementation.
